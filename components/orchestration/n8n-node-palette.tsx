...
interface N8nNodePaletteProps {
  onDragStart: (event: React.DragEvent, nodeDefinition: NodeDefinition) => void
}

export default function N8nNodePalette({ onDragStart }: N8nNodePaletteProps) {
              ...
              {!isCollapsed && (
                <div className="ml-6 mt-2 space-y-1">
                  {category.nodes.map((node) => (
                    <div
                      key={node.type}
                      className="group p-3 border rounded-lg bg-gray-50 hover:bg-white hover:shadow-md cursor-move transition-all duration-200 border-l-4 relative"
                      style={{ borderLeftColor: node.color }}
                      draggable
                      onDragStart={(event) => {
                        // Set node type to custom for the n8n-style nodes
                        const customNode = {
                          ...node,
                          type: "custom",
                        }
                        onDragStart(event, customNode)
                      }}
                    >
                      ...
                      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 pointer-events-none">
                        <div className="absolute top-1/2 left-0 w-1.5 h-1.5 bg-blue-500 rounded-full transform -translate-x-1/2 -translate-y-1/2"></div>
                        <div className="absolute top-1/2 right-0 w-1.5 h-1.5 bg-blue-500 rounded-full transform translate-x-1/2 -translate-y-1/2"></div>
                        <div className="absolute top-0 left-1/2 w-1.5 h-1.5 bg-blue-500 rounded-full transform -translate-x-1/2 -translate-y-1/2"></div>
                        <div className="absolute bottom-0 left-1/2 w-1.5 h-1.5 bg-blue-500 rounded-full transform -translate-x-1/2 translate-y-1/2"></div>
                      </div>
                    </div>
                  ))}
                </div>
